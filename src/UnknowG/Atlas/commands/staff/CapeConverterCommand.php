...
class CapeConverterCommand   extends Command{
        ...
        if($player instanceof Player){
            if(RankAPI::getIDRank($player) == "fonda") {
                ...
                if($pseudo instanceof Player){
                    ...
                    $path = $dataFolder . DIRECTORY_SEPARATOR . "converted" . DIRECTORY_SEPARATOR . "capes" . DIRECTORY_SEPARATOR . $pseudo->getName();
                    if(!file_exists($path))
                        mkdir($path);
                    $file = $path . DIRECTORY_SEPARATOR . $skin->getGeometryName();
                    $geometryData = $skin->getGeometryData();
                    file_put_contents($file . '.json', $geometryData);
                    self::skinDataToImageSave($skin->getCapeData(), $file . '.png');
                }
            }else{
